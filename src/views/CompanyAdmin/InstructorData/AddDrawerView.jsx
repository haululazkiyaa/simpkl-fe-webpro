...
import { addInstruktur } from "../../../services/instructor/instructor.service.js";
import { refreshToken } from "../../../services/auth/auth.service.js";
import { toast } from "react-toastify";
import { useNavigate } from "react-router-dom";

export default function InstructorDataAddDrawerView(props) {
  const { handleDataInstruktur, id } = props;
  ...
  const handleTambahInstruktur = (e) => {
    e.preventDefault();
    setProgress(30);
    setLoading(true);
    setMessage("");
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        addInstruktur(data, token, (status, message) => {
          if (status) {
            toast.success(`Sukses! Instruktur a.n. ${nama} ditambahakan.`, {
              autoClose: 3000,
      ...
      <Drawer title="Tambah Instruktur Baru" id={id}>
        {message != "success" ? (
          <form
            ...
            onSubmit={(e) => handleTambahInstruktur(e)}
          >
            <Input
              type="text"
              label="Username"
...
InstructorDataAddDrawerView.propTypes = {
