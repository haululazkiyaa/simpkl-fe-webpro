...
import { updateInstruktur } from "../../../services/instructor/instructor.service.js";
import { useNavigate } from "react-router-dom";

export default function InstructorDataUpdateDrawerView(props) {
  const { handleDataInstruktur, selected, id } = props;
  ...
  const handleUpdateInstruktur = (e) => {
    e.preventDefault();
    setProgress(30);
    setLoading(true);
    setMessage("");
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        updateInstruktur(data, token, (status, message) => {
          if (status) {
            toast.success(`Sukses! Data instruktur a.n. ${nama} diperbarui.`, {
              autoClose: 3000,
            ...
            setMessage(message);
            toast.error("Gagal memperbarui data instruktur!", {
              autoClose: 3000,
              hideProgressBar: false,
              closeOnClick: true,
              pauseOnHover: true,
      ...
      <Drawer title="Perbarui Data Instruktur" id={id}>
        {message != "success" ? (
          <form
            ...
            onSubmit={(e) => handleUpdateInstruktur(e)}
          >
            <Input
              type="text"
              label="Username"
              name="username"
              id="username"
              placeholder="Masukan username akun instruktur"
              value={username}
            ...
            <Input
              ...
              name="no_hp"
              id="no_hp"
              placeholder="Masukan No. HP Instruktur"
              value={noHP}
...
InstructorDataUpdateDrawerView.propTypes = {
  data: PropTypes.any,
