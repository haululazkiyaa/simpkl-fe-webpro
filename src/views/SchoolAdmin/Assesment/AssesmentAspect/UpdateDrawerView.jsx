...
import { updateAspekPenilaian } from "../../../../services/school-admin/assesment-aspect.service.js";
import { useNavigate } from "react-router-dom";

export default function AssesmentAspectUpdateDrawerView(props) {
  const { handleAspekPenilaian, selected, id } = props;
  ...
  const handleUpdateSiswa = (e) => {
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        updateAspekPenilaian(data, token, (status, message) => {
          if (status) {
            toast.success(`Sukses! Data aspek penilaian diperbarui.`, {
              autoClose: 3000,
            ...
            setMessage(message);
            toast.error("Gagal memperbarui data aspek penilaian!", {
              autoClose: 3000,
              hideProgressBar: false,
              closeOnClick: true,
              pauseOnHover: true,
  ...
  useEffect(() => {
    setKelompokPenilaian(selected?.kelompok_penilaian);
  ...
  return (
    <>
      ...
      ></button>
      <Drawer title="Update Aspek Penilaian" id={id}>
        {message != "success" ? (
          <form
          ...
          >
            ...
            <Input
              type="text"
              label="Sub Aspek Penilaian"
              name="sub_aspek_penilaian"
              id="sub_aspek_penilaian"
              placeholder="Masukan sub aspek penilaian"
              value={judul}
...
AssesmentAspectUpdateDrawerView.propTypes = {
  data: PropTypes.any,
  handleAspekPenilaian: PropTypes.func,
