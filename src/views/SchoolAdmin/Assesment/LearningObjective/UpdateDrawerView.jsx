...
import { updateTujuanPembelajaran } from "../../../../services/school-admin/learning-objective.service.js";
import { useNavigate } from "react-router-dom";

export default function LearningObjectiveUpdateDrawerView(props) {
  const { handleTujuanPembelajaran, selected, id } = props;
  ...
  const handleUpdateSiswa = (e) => {
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        updateTujuanPembelajaran(data, token, (status, message) => {
          if (status) {
            toast.success(`Sukses! Data tujuan pembelajaran diperbarui.`, {
              autoClose: 3000,
            ...
            setMessage(message);
            toast.error("Gagal memperbarui data tujuan pembelajaran!", {
              autoClose: 3000,
              hideProgressBar: false,
              closeOnClick: true,
              pauseOnHover: true,
  ...
  return (
    <>
      ...
      ></button>
      <Drawer title="Update Tujuan Pembelajaran" id={id}>
        {message != "success" ? (
          <form
...
LearningObjectiveUpdateDrawerView.propTypes = {
  data: PropTypes.any,
