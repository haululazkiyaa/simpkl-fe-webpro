...
import { updateJurusan } from "../../../services/school-admin/major-management.service.js";
import { useNavigate } from "react-router-dom";

export default function MajorManagementUpdateDrawerView(props) {
  const { handleJurusan, selected, id } = props;
  ...
  const handleUpdateJurusan = (e) => {
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        updateJurusan(data, token, (status, message) => {
          if (status) {
            toast.success(`Sukses! Jurusan ${bidangKeahlian} diperbarui.`, {
              autoClose: 3000,
  ...
  useEffect(() => {
    setBidangKeahlian(selected?.bidang_keahlian);
    setProgramKeahlian(selected?.program_keahlian);
    setKompetensiKeahlian(selected?.kompetensi_keahlian);
  }, [selected]);
        ...
        {message != "success" ? (
          ...
          >
            ...
            <Input
              ...
              value={kompetensiKeahlian}
              onChange={(e) => setKompetensiKeahlian(e.target.value)}
              required={true}
            />
            <Alert>{message}</Alert>
...
MajorManagementUpdateDrawerView.propTypes = {
  data: PropTypes.any,
