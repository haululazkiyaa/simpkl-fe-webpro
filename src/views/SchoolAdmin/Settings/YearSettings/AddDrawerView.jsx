...
export default function YearSettingsAddDrawerView(props) {
  ...
  const handleTambahTahunAjaran = (e) => {
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        addTahunAjaran(data, token, (status, message) => {
          if (status) {
            toast.success(`Sukses! Tahun ajaran ${tahunAjaran} ditambahakan.`, {
              autoClose: 3000,
        ...
        {message != "success" ? (
          ...
          >
            <Alert icon={true} variant="info">
              <strong>PERHATIAN: </strong>Penulisan tahun ajaran yang
              direkomendasikan adalah tahun ajaran diikuti semester. Contoh:
              untuk periode tahun ajaran 2023/2024 semester ganjil, maka ditulis
              2324-1.
            </Alert>
            <Input
              type="text"
              ...
              name="tahun_ajaran"
              id="tahun-ajaran"
              placeholder="Masukan tahun ajaran (Contoh: 2324-1)"
              value={tahunAjaran}
