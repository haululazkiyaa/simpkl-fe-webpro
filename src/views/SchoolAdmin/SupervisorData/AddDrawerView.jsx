...
import { addPembimbing } from "../../../services/school-admin/supervisor-data.service.js";
import { refreshToken } from "../../../services/auth/auth.service.js";
import { toast } from "react-toastify";
import { useNavigate } from "react-router-dom";

export default function SupervisorDataAddDrawerView(props) {
  ...
  const handleTambahPembimbing = (e) => {
    e.preventDefault();
    setProgress(30);
    setLoading(true);
    setMessage("");
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        addPembimbing(data, token, (status, message) => {
          if (status) {
            toast.success(`Sukses! Pembimbing a.n. ${nama} ditambahakan.`, {
              autoClose: 3000,
      ...
      <Drawer title="Tambah Pembimbing Baru" id={id}>
        {message != "success" ? (
          <form
            ...
            onSubmit={(e) => handleTambahPembimbing(e)}
          >
            <Input
              type="text"
              label="NIP"
...
SupervisorDataAddDrawerView.propTypes = {
