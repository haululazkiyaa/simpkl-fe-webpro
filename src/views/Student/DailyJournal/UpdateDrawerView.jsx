...
import { updateJurnalHarian } from "../../../services/student/daily-journal.service.js";
import { useNavigate } from "react-router-dom";

export default function StudentDailyJournalUpdateDrawerView(props) {
  ...
  const handleUpdateJurnalHarian = (e) => {
    e.preventDefault();
    setProgress(30);
    setLoading(true);
    setMessage("");
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        updateJurnalHarian(data, token, (status, message) => {
          if (status) {
            toast.success(`Sukses! Jurnal harian diperbarui.`, {
              autoClose: 3000,
            ...
            setMessage(message);
            toast.error("Gagal memperbarui jurnal harian!", {
              autoClose: 3000,
              hideProgressBar: false,
              closeOnClick: true,
              pauseOnHover: true,
      ...
      <Drawer title="Catatan Jurnal" id={id}>
        {message != "success" ? (
          <form
            ...
            onSubmit={(e) => handleUpdateJurnalHarian(e)}
          >
            {hari != "" && (
              <SelectInput
                options={opsiHari}
                ...
                defaultValue={opsiHari.find(
                  ({ value }) => value === selected.hari
                )}
                isDisabled={true}
              />
            )}
            ...
            {jenisPekerjaan != "" && (
              <SelectInput
                ...
                defaultValue={opsiJenisPekerjaan.find(
                  ({ value }) => value === selected.jenis_pekerjaan
                )}
              />
            )}
            <TextArea
            ...
            {bentuKegiatan != "" && (
              <SelectInput
                ...
                defaultValue={opsiBentukKegiatan.find(
                  ({ value }) => value === selected.bentuk_kegiatan
                )}
              />
            )}
            <Input
...
StudentDailyJournalUpdateDrawerView.propTypes = {
  data: PropTypes.any,
