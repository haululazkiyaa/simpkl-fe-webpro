import {
  addNilaiBulananPembimbing,
  getNilaiBulananPembimbing,
} from "../../../services/supervisor/supervisor-monthly-grade.service.js";
...
export default function SupervisorMonthlyAssesmentGradeView(props) {
  ...
  }, [setProgress, navigate, selected, bulan, tahun]);

  const handleDeskripsi = (item, value) => {
    const newData = detail.map((data) => {
      if (data.id_tujuan_pembelajaran === item.id_tujuan_pembelajaran) {
        return { ...data, deskripsi: value };
      }
      return data;
    });
  ...
  const handleSimpanNilai = (e) => {
    ...
    refreshToken((status, token) => {
      if (status) {
        setProgress(60);
        addNilaiBulananPembimbing(data, token, (status, message) => {
          if (status) {
                  ...
                  >
                    ...
                    <td className="w-28 px-3">
                      <Input
                        ...
                        placeholder="Nilai"
                        value={item.nilai}
                        onChange={(e) => handleNilai(item, e.target.value)}
                      />
                    </td>
                    <td className=" w-72 p-3">
                      <TextArea
                        ...
                        onChange={(e) => handleDeskripsi(item, e.target.value)}
                        required={true}
                      />
                    </td>
                  </tr>
